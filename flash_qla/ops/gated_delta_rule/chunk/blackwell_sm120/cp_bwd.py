...
def tilelang_prepare_dh_ws(
    ...
    def tilelang_prepare_dh_kernel(
        ...
        with T.Kernel(batch_size * H, threads=512) as (bbh,):
            ...
            # sm120 smem opt-in cap is 101376B: do_shared is single-buffered
            # (saves 8KB vs staging); its reuse is guarded by bar_do_free.
            ...
            # bar_do_free: Consumer_dh finished reading do_shared (128);
            # P1 waits on it before overwriting the single-buffered do_shared.
            bar_do_free = T.alloc_barrier(arrive_count=128)
            ...
            if tx < 128:
                ...
                for i_s in T.serial(num_iters):
                    ...
                    # do_shared consumed: allow P1 to overwrite it
                    T.barrier_arrive(bar_do_free)
                ...
                if tx < 384 + 32:
                    ...
                    for i_s in T.serial(num_iters):
                        ...
                        # do_shared is single-buffered: wait until Consumer_dh
                        # finished the previous iteration's do^T @ q gemm.
                        if i_s > 0:
                            T.barrier_wait(bar_do_free, (i_s - 1) % 2)
                        chunk_idx = num_iters - 1 - i_s
                        left = seq_start_idx + chunk_idx * block_S
                        right = left + block_S
                        ...
                        if right <= seq_end_idx:
                            ...
                            for j_s, j_v in T.Parallel(block_S, DV):
                                if left + j_s < seq_end_idx:
                                    do_shared[j_s, j_v] = do[batch_idx, left + j_s, bh, j_v]
                                else:
                                    do_shared[j_s, j_v] = 0
                        if right <= seq_end_idx:
                            T.tma_copy(
